...
<template>
  <Head title="Solo Bracket | TM7 Club" />

  <PublicLayout>
    <div class="max-w-7xl mx-auto px-6 py-12 lg:py-24 overflow-x-hidden">
      
      <div class="mb-10 text-center relative z-10">
        <h2 class="text-3xl font-serif font-bold text-white mb-3 tracking-wide">Individual Tournament Bracket</h2>
        <p class="text-gray-400">Follow the progress of the solo competition.</p>
      </div>
            ...
            <Bracket :rounds="tournamentData">
               <template #player="{ player }">
                   <div :class="['relative px-4 py-2 border rounded-md shadow-md min-w-[180px] text-center font-medium transition-all group', player.winner ? 'bg-tm7-gold text-tm7-darker border-tm7-gold' : 'bg-tm7-darker border-white/10 text-gray-300']">
                       <a v-if="player.email" :href="'mailto:' + player.email + '?cc=' + $page.props.admin_cc_email" class="hover:underline transition-all" title="Email Player">
                           {{ player.name }}
                       </a>
