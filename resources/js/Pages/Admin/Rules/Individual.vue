...
const form = useForm({
    individual_rules: props.rules,
});

const submit = () => {
    form.post(route('admin.rules.update'), {
        preserveScroll: true,
        onSuccess: () => alert('Individual Rules updated!'),
    });
};
</script>

<template>
    <Head title="Edit Individual Rules" />
            ...
            <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Changes here will be instantly visible on the public Individual Rules page.</p>
                    </div>
                    <button 
                        @click="submit"
                ...
                <div class="space-y-4">
                    ...
                    <Editor v-model="form.individual_rules" editorStyle="height: 500px" />
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
