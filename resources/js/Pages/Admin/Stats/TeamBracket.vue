...
const generateBracket = () => {
    if (confirm('Regenerate Team Bracket? This will delete current team matches.')) {
        form.post(route('admin.seed'), { preserveScroll: true });
    }
};

const publishToggle = useForm({ type: 'team' });
const togglePublish = () => {
...
const advanceForm = useForm({ type: 'team', round: 1 });
...
const confirmDeleteBracket = () => {
    router.delete(route('admin.bracket.delete', 'team'), {
        onSuccess: () => {
...
<template>
    ...
    <Head title="Team Bracket Management" />

    <AuthenticatedLayout>
        <template #header>Team Bracket Management</template>
        ...
        <div class="space-y-8">
            ...
            <div
                ...
                <div class="space-y-1">
                    <h3 class="font-bold text-gray-900">Bracket Controls</h3>
                    <p class="text-xs text-gray-500">Manage visibility and registration for the Team Tournament.</p>
                </div>
        ...
        <ConfirmModal :show="showingDeleteBracketConfirm" title="Delete Entire Bracket?"
            message="Are you sure you want to delete all team matches? <br><br>A **backup snapshot** will be saved in the Backup Brackets menu, so you can restore it later if needed. However, registration will be unlocked immediately."
